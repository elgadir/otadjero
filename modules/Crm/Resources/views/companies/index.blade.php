...
@section('new_button')
    ...
    <span>
        <a href="{{ route('import.create', ['crm', 'companies']) }}" class="btn btn-white btn-sm header-button-top">
            {{ trans('import.import') }}
        </a>
    </span>
    @endpermission
            ...
            <div class="card-header border-bottom-0" :class="[{'bg-gradient-primary': bulk_action.show}]">
                {!! Form::open([
                    ...
                    <div class="align-items-center" v-if="!bulk_action.show">
                        <x-search-string model="Modules\Crm\Models\Company" />
                    </div>

                    {{ Form::bulkActionRowGroup('crm::general.companies', $bulk_actions, ['group' => 'crm', 'type' => 'companies']) }}
                {!! Form::close() !!}
            </div>
                        ...
                        @foreach($companies as $item)
                            <tr class="row align-items-center border-top-1">
                                ...
                                <td class="col-xs-4 col-sm-3 col-md-3 col-lg-3 col-xl-2">
                                    <a class="col-aka text-success" href="{{ route('crm.companies.show', $item->id) }}">{{ $item->name }}</a>
                                </td>
                                ...
                                <td class="col-sm-3 col-md-2 col-lg-2 col-xl-2 d-none d-sm-block long-texts text-left">
                                    {{ !empty($item->contact->phone) ? $item->contact->phone : trans('general.na') }}
                                </td>
                                ...
                                <td class="col-xs-4 col-sm-2 col-md-2 col-lg-1 col-xl-1 text-center">
                                    @if (user()->can('update-crm-companies'))
                                        {{ Form::enabledGroup($item->contact->id, $item->name, $item->contact->enabled) }}
                                    @else
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                            ...
                                            @permission('delete-crm-companies')
                                                {!! Form::deleteLink($item, 'crm.companies.destroy', 'crm::general.companies') !!}
                                            @endpermission
                                        </div>
                                    </div>
                                </td>
