...
@section('new_button')
    ...
    <span>
        <a href="{{ route('import.create', ['crm', 'contacts']) }}" class="btn btn-white btn-sm header-button-top">
            {{ trans('import.import') }}
        </a>
    </span>
    @endpermission
...
@section('content')
    @if ($contacts->count() || request()->get('search', false))
        <div class="card">
            <div class="card-header border-bottom-0" :class="[{'bg-gradient-primary': bulk_action.show}]">
                {!! Form::open([
                    ...
                    <div class="align-items-center" v-if="!bulk_action.show">
                        <x-search-string model="Modules\Crm\Models\Contact" />
                    </div>

                    {{ Form::bulkActionRowGroup('crm::general.contacts', $bulk_actions, ['group' => 'crm', 'type' => 'contacts']) }}
                {!! Form::close() !!}
            </div>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                            ...
                                            @permission('delete-crm-contacts')
                                                {!! Form::deleteLink($item, 'crm.contacts.destroy' , 'crm::general.contacts') !!}
                                            @endpermission
                                        </div>
                                    </div>
                                </td>
...
@push('scripts_start')
    <script src="{{ asset('modules/Crm/Resources/assets/js/contacts.min.js?v=' .module_version('crm')) }}"></script>
@endpush
