...
class Users extends ApiController
{
    ...
    {
        $user = auth()->user();
        $users = User::with('companies', 'permissions', 'roles', 'warehouses')
                     ->where('id', $user->id)
                     ->get();
    
        $paginator = new LengthAwarePaginator($users, count($users), 1);
        
        return $this->response->paginator($paginator, new Transformer());
    }
